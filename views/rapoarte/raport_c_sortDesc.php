...
function showRaportAsc() {
	...
	$query_result = $db_connection->query("SELECT emp_id, project_id, SUM(hours) suma, SUM(extra_hours) suma_extra, SUM(hours) + SUM(extra_hours) suma_total 
							FROM (SELECT * from timesheet where project_id in (SELECT id from projects where dept_id = '" . $_SESSION['dept_id'] . "')) as x where date BETWEEN '". $_SESSION['c_start_date'] ."' AND '". $_SESSION['c_end_date'] ."' GROUP BY project_id ORDER BY suma_total DESC;");
		...
		$r .= '         <th style="text-align:center" width="75	 align="center">Nume Proiect</td>';
        $r .= '         <th style="text-align:center" width="75	 align="center">Ore Lucrate</td>';
        $r .= '         <th style="text-align:center" width="75	 align="center">Ore Lucrate Extra</td>';
        $r .= '         <th style="text-align:center" width="75	 align="center">Ore Lucrate Total</td>';
        $r .= '    </tr>';
		$r .= '</thead>';
